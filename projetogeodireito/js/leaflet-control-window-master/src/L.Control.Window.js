L.Control.Window = L.Control.extend({
  ...
  initialize: function (container, options) {
    ...
    this._wrapper = L.DomUtil.create(
      "div",
      modality + " leaflet-control-window-wrapper",
      L.DomUtil.get(this.options.element)
    );

    this._container = L.DomUtil.create(
      "div",
      "leaflet-control leaflet-control-window " + this.options.className,
      this._wrapper
    );
    ...
    this._containerTitleBar = L.DomUtil.create(
      "div",
      "titlebar",
      this._container
    );
    this.titleContent = L.DomUtil.create(
      "h2",
      "title",
      this._containerTitleBar
    );
    this._containerContent = L.DomUtil.create(
      "div",
      "content",
      this._container
    );
    this._containerPromptButtons = L.DomUtil.create(
      "div",
      "promptButtons",
      this._container
    );
    ...
    L.DomEvent.on(this._wrapper, "contextmenu", stop)
  ...
  show: function (position) {
    ...
    if (this.options.position == "topLeft") {
      ...
      this.showOn([
        left + width - thisWidth - margin * 2,
        top + height / 2 - thisHeight / 2 - margin + offset,
      ]);
    } else if (this.options.position == "bottomRight") {
      this.showOn([
        left + width - thisWidth - margin * 2,
        top + height - thisHeight - margin * 2 - offset,
      ]);
    } else if (this.options.position == "bottom") {
      this.showOn([
        left + width / 2 - thisWidth / 2 - margin,
        top + top + height - thisHeight - margin * 2 - offset,
      ]);
    } else {
      /* center = map.getCenter();
      this.showOn([center.lat, center.lng]);
      console.log(this.showOn([center.lat, center.lng])); */
      this.showOn([
        left + width / 2 - thisWidth / 2 - margin,
        top + height / 2 - thisHeight / 2 - margin + offset,
      ]);
  ...
  showOn: function (point) {
    /* L.DomUtil.setPosition(
      this._container,
      L.point(Math.round(point[0]), Math.round(point[1]), true) */
    this.setContentMaxHeight();
    console.log(document.getElementById("page-instrucoes"));
    L.DomUtil.setPosition(
      this._container,
      L.point(Math.round(point[0]), Math.round(point[1]), false)
    );
  ...
  prompt: function (promptObject) {
    ...
    if (action != undefined) {
      var btnAction = L.DomUtil.create(
        "button",
        "",
        this._containerPromptButtons
      );
    ...
    if (cancel != undefined) {
      var btnCancel = L.DomUtil.create(
        "button",
        "",
        this._containerPromptButtons
      );
  ...
  setContentMaxHeight: function () {
    ...
    this._containerContent.setAttribute(
      "style",
      "min-height:80%"
    );
  },
